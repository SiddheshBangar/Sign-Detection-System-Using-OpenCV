...
while 1:
    ...
    ret1, thresh2 = cv2.threshold(
        blurred1, 127, 255, cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)
    ...
    contours1, hierarchy1 = cv2.findContours(
        thresh2.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
    max_area1 = -1
    ...
    for l in range(defects1.shape[0]):
        ...
        # draw a line from start to end i.e. the convex points (finger tips)qqqqqq
        # (can skip this part)
    ...
    if cv2.waitKey(1) == ord('w'):
        if (count_defects == 1 and count_defects1 == 0):
            cv2.putText(img, "_", (200, 50), cv2.FONT_HERSHEY_COMPLEX, 2, 2)
